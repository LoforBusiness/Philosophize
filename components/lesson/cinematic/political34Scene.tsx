...
export default function Political34Scene({ clock, bt, bi, i, dragPos, pickPos, gazeX, gazeY, gazeOn }: SceneApi) {
  const heldS = useHeld();
  const cv = useCarry(3);
  ...
  const SCENE = useDerivedValue(() => {
    ...
    return {
      ...
      level: live ? pickPos.value : carry(cv, 0, n, LEVEL[p], LEVEL[n], widen),
