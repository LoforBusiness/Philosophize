...
export const LOAFER_LINES = [
  ...
  // ── THE SECOND HUNDRED ────────────────────────────────────────────────────
  //
  // Same voice, same box — and the breaks here are NOT hand-typed. Every one was
  // written flat and then broken BY MEASUREMENT: scripts/lib/ttfwidth.mjs reads the
  // real advance widths out of Inter's own .ttf, and a scorer enumerates every legal
  // breaking and takes the fewest rows first, then the most even, then the ones
  // landing on a full stop. `npm run check:quips` re-measures all of them.
  //
  // That is not ceremony, and the comment above understates the trap. A character
  // count is not a width — "Wittgenstein" is 78px here and "illiterate," is 51, at
  // twelve characters each — so the widest row that fits is 22 characters and the
  // narrowest that does not is 20. And the cloud is anchored from its BOTTOM so that
  // it hangs at his head, which means a row too many does not push anything down: it
  // grows UP, off the top of the block.
  //
  // They are broken to 122px rather than the box's 125. The measurer ignores
  // kerning, which is nearly always negative, so a row measured at 124.9 fits by a
  // margin thinner than the difference between two text engines — and the first
  // hundred already top out at 123.3, so that headroom is the house's own.
  'Aristotle called\nthis leisure.\nHe had staff.',
  'You have now\nread more than\nmost emperors.',
