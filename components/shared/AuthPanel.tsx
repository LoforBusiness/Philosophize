...
// The combined sign-in / create-account form. Used both as the first screen on
// launch (app/index.tsx, when there's no Supabase session) and as an in-app
// route reached from the Profile "Sign in" CTA (app/sign-in.tsx). The X (and the
// "Continue without an account" link) go back if there's somewhere to return to,
// otherwise they drop the user into the app as a guest — the app is fully usable
// locally. A successful sign-in/sign-up is routed into the app by the auth
// listener in app/_layout.tsx, so this panel never navigates on success itself.
export default function AuthPanel() {
  ...
  function skip() {
    // Reached in-app (e.g. from the Profile CTA) → dismiss back; reached as the
    // launch screen → there's nowhere to go back to, so enter as a guest.
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView
        ...
        style={{ flex: 1 }}
      >
        {/* Skip — continue without an account / dismiss */}
        <Pressable
          onPress={skip}
          hitSlop={10}
