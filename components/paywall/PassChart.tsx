...
import { METAL, INK, MID, GHOST, PAPER, PAPER_LIT, PAPER_SHADE, mix } from '@/components/shared/tone';
...
// raised column is struck GOLD on a hard lip, the metal every reward in this app
// is made of, and a cross only appears where the free tier has none of the thing.
//
// ── THE ARRIVAL IS OPTIONAL, AND THAT IS THE READER'S CALL ──────────────────
//
// Pass a `play` driver and the chart arrives: a glint crosses the gold, then the
...
export const INK_GOLD = mix(GOLD.base, INK, 0.34);
...
interface Metrics {
  ...
  /** Gold showing round the white card, and the hard lip under the column. */ frame: number; lip: number;
  ...
  if (cell.kind === 'yes') {
    return (
      <View style={s.coinHalo}>
        <LinearGradient colors={[GOLD.lit, GOLD.base]} start={LIGHT_START} end={LIGHT_END} style={s.coin}>
...
/** The glint across the gold, once per arrival. Clipped by the frame's own corners. */
...
function sized(m: Metrics) {
  ...
  return StyleSheet.create({
    // It runs under the gold column, so its right-hand corners are tucked behind
    ...
    passLip: {
      ...
      borderRadius: m.radius, backgroundColor: GOLD.rim,
    },
    passFace: {
      position: 'absolute', left: 0, right: 0, top: 0, bottom: m.lip,
      borderRadius: m.radius, overflow: 'hidden', borderWidth: 1, borderColor: GOLD.shade,
    },
    passCard: {
      position: 'absolute', top: m.headH - 1, left: m.frame - 1, right: m.frame - 1, bottom: m.frame - 1,
      borderRadius: m.cardRadius, backgroundColor: PAPER_LIT, overflow: 'hidden',
    },
    ...
    headPass: { fontFamily: 'PlayfairDisplay_700Bold', fontSize: m.headPass, color: GOLD.on, includeFontPadding: false },
    ...
    coin: {
      width: m.mark, height: m.mark, borderRadius: m.mark / 2, borderWidth: 1.5, borderColor: GOLD.shade,
      alignItems: 'center', justifyContent: 'center',
    },
    ...
    tickShort: {
      position: 'absolute', width: 9 * k, height: 2.6 * k, borderRadius: 1.3 * k, backgroundColor: INK,
      transform: [{ translateX: -4.25 * k }, { translateY: 1.75 * k }, { rotate: '45deg' }],
    },
    tickLong: {
      position: 'absolute', width: 14 * k, height: 2.6 * k, borderRadius: 1.3 * k, backgroundColor: INK,
