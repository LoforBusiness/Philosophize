...
export function TheWall({
  ...
  return (
    <View style={s.wall}>
      ...
      <View style={s.tickBox}>
        <View style={s.tickRun}>
          {/* One mark, struck in the palette’s teal, for the one sitting it would take. Same
          ...
          {/* STRUCK FROM THE SHADED HALF OF THE METAL, not the lit half. Gold's
              ...
              a second grey tick. */}
          <LinearGradient
            colors={[PATINA.base, PATINA.shade, PATINA.rim]}
            start={LIGHT_START}
            end={LIGHT_END}
            style={s.tickLit}
          />
...
export function LibraryLine({
  lessonsByBranch, tint = PATINA.base,
}: {
    ...
    <View style={s.table}>
      ...
      <View style={s.headRow}>
        ...
        <View style={{ width: COL, alignItems: 'center' }}>
          <MetalPlate metal={PATINA} label="THE PASS" />
        </View>
      </View>
