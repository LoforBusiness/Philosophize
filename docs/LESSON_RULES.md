...
discovering something"*, plus a line back when they answer — teasing on a wrong
one, *"passive aggressive that is somewhat encouraging"* on a right one. **The
point is that he is learning too.** He is not presenting the material; he is the
other student.
...
186 must-box measurements stale and demands hours of browser time to record
something the stage never draws from the script.

### AB2 · A bubble never covers a WORD; art is negotiable (D31)

Held to "clear of everything", a bubble has nowhere to go in **41 of 186
lessons** — these stages are built to fill the frame, which is what the tonal
mass pass was for. Held to D31 — *"nothing painted over a word is acceptable,
including the app's own decoration"* — it fails in **13**. A word covered for two
seconds is a word the reader does not get; a corner of a diagram behind a small
opaque box is what every comic has always done. The generator still PREFERS a
fully clear spot and only spends the licence when it must.

### AB3 · Three parameters, and each was worth lessons

- **Height.** A one-row thought needs 34 units and a two-row one 48. That
  difference alone is worth eleven lessons, so the generator wraps the REAL
  string against the real `.ttf` and places the box that beat will actually draw.
- **The trail.** Three discs cost 27.5 units, one costs 7. `logic-arguments-3`
...
Loop order is a way of expressing a preference, and it expressed the wrong one
twice. Trail outermost walked a bubble 120 units sideways into the middle of an
argument card — covering no word, inside the band, and reading as a LABEL ON THE
CARD. Mode outermost preferred a fully clear spot 84 units above his crown to one
4 units above that grazed a diagram, and 84 units of blank paper between the last
disc and his head reads as a caption that has come loose.

Every candidate is costed now and the cheapest wins: `up * 2 + |dx| + 26 if over
art + 6 per missing disc`. **A priority expressed by nesting is a priority nobody
can read or change.**

### AB5 · A measurement is spent in the space it was taken in

The first version emitted an OFFSET from the figure and let the player add it to
the figure's live x — which the player derives from the script's `walk` track,
which a scene need not pass. Where it was missing the bubble fell back to the
...
### AB7 · He mocks the answer, never the answerer

§7's rule carried into a fourth pool: *"make fun of them"* was put to the reader
rather than guessed, and the answer was that the joke lands on the CHOICE, the
question, or HIM — *"Ah. I'd have said that too."*, *"That's the trap. Good
trap."*, *"Everyone picks that one."* The person who comes off worst is the
mascot. A beginner who has just been wrong is the reader most likely to close the
app, and the line that lands on them there is the one they remember.

**And a hash that indexes an array must be clamped.** `quipFor` returned
`undefined` for about a third of all seeds, because `^` evaluates as signed
int32 and the finaliser's last xor was never re-clamped with `>>> 0`. It would
have shipped as an empty bubble. Three other files in the repo get this right;
`liven-lessons` carried the same latent copy.

### AB8 · Nothing over his head while the reader is still choosing (group O)
