...
// The probe itself is in ./lib/mustprobe.mjs, so validate-cinematic can hash the
// same text when it decides whether a stored box is stale — see ./lib/muststamp.mjs.
...
import { setThoughtsOff, setToursOff, setCodaOff, setWanderOff } from '@/components/lesson/cinematic/tourFlag';
 ...
 *
 * It hashes the PROBE as well as the scene, and ./lib/muststamp.mjs is one copy
 * of that rule rather than two. The reason is in that file: an older probe
 * recorded about one word a beat, every stamp stayed green, and the tour
 * generator pushed to 1.68x straight over labels it had no record of.
 */
