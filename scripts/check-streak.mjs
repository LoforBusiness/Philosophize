...
head('THE STREAK PURPLE, MEASURED');
{
  ...
  const claims = [
    ...
    // These two exist for that ground and are checked against it, never paper.
    ['STREAK_ON_INK on ink', streak.STREAK_ON_INK, INK, 4.5],
  ...
  else bad('purple and slate are too close to tell apart', `ΔE ${dE.toFixed(1)}`);
  ...
  const [L3, a3, b3] = lab(streak.STREAK_ON_INK);
  const [L4, a4, b4] = lab(streak.SLATE_LIT);
  const dLit = Math.hypot(L3 - L4, a3 - a4, b3 - b4);
  if (dLit > 20) ok('purple and slate stay different on a dark ground too',
    `ΔE ${dLit.toFixed(1)}`);
  ...
  const METALS = { AURUM: '#916D0F', BRONZE: '#874B21', AMETHYST: '#6E3396' };
  for (const [name, hex] of Object.entries(METALS)) {
    const [ml, ma, mb] = lab(hex);
    const d = Math.hypot(L1 - ml, a1 - ma, b1 - mb);
    if (d >= 15) ok(`the purple is tellable from ${name}, which it meets on Profile`, `ΔE ${d.toFixed(1)}, floor 15`);
    else bad(`the purple reads as ${name}`, `ΔE ${d.toFixed(1)}, needs 15`);
  }
  ...
  // THE STREAK'S TONES ARE THE PALETTE'S, NOT A SECOND COPY OF IT. This loader
  // cannot follow an import, so constants/streak.ts writes the four out as hex;
...
//
// THE RAIL WAS ONCE DRAWN IN THE STREAK'S WASH AND COULD NOT BE SEEN. It measured
...
head('THE MASCOT');
{
  ...
  // THE GLOW DECAYS, AND NEVER TO NOTHING WHILE THE STREAK LIVES. A flame that
  ...
  if (at(23) >= mood.GLOW_FLOOR - 1e-9) ok('and never below the floor while alive', `floor ${mood.GLOW_FLOOR}`);
