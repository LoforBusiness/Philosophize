...
//
// It copies one narrated lesson (its clips, its manifest block and its render records)
// into a temporary folder, damages one thing at a time, and runs the real check on the
// copy through NARRATION_ONLY, NARRATION_MANIFEST and NARRATION_ASSETS. A red run only
// counts if it names the KIND of finding the damage should cause, so a stage cannot pass
// because something else happened to go red, and a stage that names kinds which must
// NOT appear proves each audio limit fires on its own.
//
...
import { ROOT, ASSETS, MANIFEST, parseWav, sha256hex, readRenders, writeRenders } from './lib/narration.mjs';
...
function run() {
  try {
    const out = execFileSync(process.execPath, ['scripts/check-narration.mjs'], {
      cwd: ROOT,
      encoding: 'utf8',
      env: { ...process.env, NARRATION_ONLY: LESSON, NARRATION_MANIFEST: stagedManifest, NARRATION_ASSETS: stagedAssets },
    });
...
const mp3File = (beat) => path.join(stagedAssets, LESSON, `${nn(beat)}.mp3`);
...
/**
 * Rewrite one staged WAV. By default its MP3 tag and its render record follow it, so
 * only the damage itself can be what the check reports.
 */
function damage(beat, fn, { mp3 = true, record = true } = {}) {
  ...
  if (mp3) {
    const m = fs.readFileSync(mp3File(beat));
    const at = m.indexOf(`wav-sha256:${sha256hex(before)}`);
    if (at < 0) throw new Error(`${nn(beat)}.mp3 carries no tag for its WAV`);
    Buffer.from(`wav-sha256:${sha256hex(after)}`).copy(m, at);
    fs.writeFileSync(mp3File(beat), m);
  }
...
// ── the take is the one recorded, for these words, and its MP3 is made from it ─────
fresh();
damage(0, (pcm) => { pcm[1000] += 1; return pcm; }, { mp3: false });
expect('a WAV changed after its MP3 was encoded', 'STALE MP3');
