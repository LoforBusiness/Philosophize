// RE-STAMP THE MUST-BOXES, THE TOURS AND THE MARK AUDIT AFTER `lip-stage` GAVE EVERY
// TONED PLATE ITS SHADED LIP.
//
//   node scripts/restamp-lip.mjs [--write]
//
// `restamp-hue.mjs` with the lip codemod in place of the hue one, and the same two-part
// proof, which is the whole of its safety: re-running `lipScene` on the COMMITTED text
// must reproduce the file on disk byte for byte, and the stored stamp must match the
// stamp of that committed text. A `boxShadow` changes no layout and no bounding box —
// the renderer draws it behind the element — so a proven scene's measurements are
// exactly as good as they were.
//
// One store more than the hue pass had: scripts/lib/markEdges.json, the painted boxes
// round each pen-mark candidate. A shadow is not a background or a border, so the
// audit reads the same boxes before and after, and its stamps move with the rest.
//
...
for (const file of fs.readdirSync(DIR).filter((f) => f.endsWith('Scene.tsx'))) {
  ...
  const out = lipScene(was);
  if (out.skip) { unproven.push([file, `codemod would skip it (${out.skip}) yet it changed`]); continue; }
  if (out.src !== now) { unproven.push([file, 'differs from HEAD by more than the lip']); continue; }
  committed.set(full, was);
}
console.log(`${committed.size} scene(s) proven to differ from HEAD by the lip alone`);
if (unproven.length) {
